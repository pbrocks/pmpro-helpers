...
// use PMPro_Helpers\inc\library\Requests as Requests;
...
class PMPro_Get_Gists {
	...
	public static function init() {
		add_action( 'admin_menu', array( __CLASS__, 'pmpro_gists' ) );
		// add_action( 'admin_enqueue_scripts', array( __CLASS__, 'pmpro_gists_enqueue' ) );
		// add_filter( 'plugin_action_links_' . plugin_basename( __FILE__ ), 'pmpro_gists_action_links' );
		// add_filter( 'pmpro_menu_title', array( __CLASS__, 'pmpro_change_menu_name' ) );
	}

	/**
	 * Add a page to the gists menu.
	 *
	 * @since 1.0.0
	 *
	 * @return array
	 */
	public static function pmpro_gists() {
		global $this_menu_slug;
		$this_menu_slug = preg_replace( '/_+/', '-', __FUNCTION__ );
		$label = ucwords( preg_replace( '/_+/', ' ', __FUNCTION__ ) );
		// $pmpro_gists_screen = add_submenu_page( __( 'pmpro-membershiplevels', $label, 'pmpro-gists-menu' ), __( $label, 'pmpro-gists-menu' ), 'manage_options', $this_menu_slug . '.php', array( __CLASS__, 'pmpro_gists_page' ) );
		$pmpro_gists_screen = add_dashboard_page( __( $label, 'pmpro-gists-menu' ), __( $label, 'pmpro-gists-menu' ), 'manage_options', $this_menu_slug . '.php', array( __CLASS__, 'pmpro_gists_page' ) );
		// Adds my_help_tab when gists_page loads
		add_action( 'load-' . $pmpro_gists_screen, array( __CLASS__, 'add_gists_help_tab' ) );
	}

	public static function add_gists_help_tab() {
	...
	public static function pmpro_gists_enqueue() {
		wp_register_script(
			'basic-accordion',
	 ...
	 * @return
	 */
	public static function pmpro_gists_page() {
		global $level, $pmpro_levels;
		echo '<div class="wrap">';
		echo '<h2>' . ucwords( preg_replace( '/_+/', ' ', __FUNCTION__ ) ) . '</h2>';
		$screen = get_current_screen();
		echo '<h2>' . plugin_dir_path( __DIR__ ) . 'library/Requests.php</h2>';
		// Require the Require Library
		// require_once plugin_dir_path( __DIR__ ) . 'library/Requests.php';
		// \Requests::register_autoloader();
		// Setup request to a WP site
		$response = \Requests::get( 'https://api.github.com/users/pbrocks/gists/?per_page=15' );
		// Convert JSON to PHP Array
		$gists = json_decode( $response->body, true );
		echo '$gists <pre>';
		print_r( $gists );
		echo '</pre>';
		/*
			  ?>
		<div id="gists-accordion">
	 ...
	 */
	public static function pmpro_gists_action_links( $links ) {
		global $this_menu_slug;
		if ( current_user_can( 'manage_options' ) ) {
			$new_links = array(
				'<a href="' . get_admin_url( null, 'index.php?page=' . $this_menu_slug ) . '">' . __( 'View gists', 'pmpro-gists-menu' ) . '</a>',
			);
		}
